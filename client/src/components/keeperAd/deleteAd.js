...
const DeleteAd = (props) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios({
                method: 'DELETE',
                url: `/keeperads/${props.id}`,
                headers: {
                    'x-auth-token': props.user.token
                },
            })
            props.setAllKeepers(props.allKeepers.filter((keeper) => keeper._id !== props.id))
            props.setAllKeepersfiltered(props.allKeepersfiltered.filter((keeper) => keeper._id !== props.id))
          Swal.fire(
            'Deleted!',
